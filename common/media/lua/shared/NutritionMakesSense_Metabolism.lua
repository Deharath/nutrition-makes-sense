...
Metabolism.FUEL_LOW_THRESHOLD = 500
Metabolism.FUEL_PENALTY_THRESHOLD = 200
Metabolism.DEFAULT_FUEL = 1000

Metabolism.DEPRIVATION_ONSET = 0.25
...
Metabolism.MACRO_MAX = {
    carbs = 1400,
    fats = 420,
    proteins = 350,
}

Metabolism.DEFAULT_MACROS = {
    carbs = 980,
    fats = 294,
    proteins = 245,
}

Metabolism.MACRO_DEFICIENCY_START_RATIO = {
    carbs = 0.10,
    fats = 0.10,
    proteins = 0.10,
}

Metabolism.MACRO_MAX_EFFECT = {
    carb_endurance = 0.18,
    fat_weight_loss = 0.18,
    protein_healing = 0.12,
}

Metabolism.DEFAULT_WEIGHT_KG = 80
Metabolism.WEIGHT_MIN_KG = 35
Metabolism.WEIGHT_MAX_KG = 140
Metabolism.WEIGHT_KCAL_PER_KG = 7700
Metabolism.WEIGHT_RESPONSE_HOURS = 8
Metabolism.WEIGHT_CONTROLLER_RESPONSE_HOURS = 6

Metabolism.SLEEP_FUEL_BURN_PER_HOUR = 35
Metabolism.SLEEP_VISIBLE_HUNGER_PER_HOUR = 0.004
Metabolism.SLEEP_MACRO_BURN_PER_HOUR = {
    carbs = 4.0,
    fats = 1.2,
    proteins = 0.8,
}
...
Metabolism.SATIETY_BUFFER_DECAY_PER_HOUR = 0.25
...
Metabolism.BASE_WAKE_HUNGER_PER_HOUR = 0.032
...
function Metabolism.getFuelBurnPerHourFromMet(workload)
    local summary = Metabolism.normalizeWorkload(workload)
    if summary.sleepObserved then
        return Metabolism.SLEEP_FUEL_BURN_PER_HOUR
    end
    return 75 + (48 * math.max(0, summary.averageMet - Metabolism.MET_REST))
end
...
function Metabolism.getMacroBurnPerHourFromMet(workload)
    local summary = Metabolism.normalizeWorkload(workload)
    if summary.sleepObserved then
        return {
            carbs = Metabolism.SLEEP_MACRO_BURN_PER_HOUR.carbs,
            fats = Metabolism.SLEEP_MACRO_BURN_PER_HOUR.fats,
            proteins = Metabolism.SLEEP_MACRO_BURN_PER_HOUR.proteins,
        }
    end
    local metDelta = math.max(0, summary.averageMet - Metabolism.MET_REST)
    return {
        carbs = 10.0 + (1.0 * metDelta),
        fats = 3.0 + (0.2 * metDelta),
        proteins = 2.0 + (0.133333 * metDelta),
    }
end
...
function Metabolism.getMacroDeficiencyProgress(macroName, reserve)
    local maxReserve = Metabolism.MACRO_MAX[macroName]
    if not maxReserve or maxReserve <= 0 then
        return 0
    end

    local startRatio = Metabolism.MACRO_DEFICIENCY_START_RATIO[macroName] or 0.10
    local deficiencyStart = maxReserve * startRatio
    local available = clamp(reserve or 0, 0, maxReserve)
...
function Metabolism.getCarbEnduranceMultiplier(carbs, effectiveEnduranceMet)
    local deficiency = Metabolism.getMacroDeficiencyProgress("carbs", carbs)
    return lerp(1.0, 1.0 + Metabolism.MACRO_MAX_EFFECT.carb_endurance, deficiency)
end

function Metabolism.getFatWeightLossMultiplier(fats, energyBalanceKcal, weightController)
    if (tonumber(weightController) or 0) >= 0 or (tonumber(energyBalanceKcal) or 0) >= 0 then
        return 1.0
    end

    local deficiency = Metabolism.getMacroDeficiencyProgress("fats", fats)
    return lerp(1.0, 1.0 + Metabolism.MACRO_MAX_EFFECT.fat_weight_loss, deficiency)
end

function Metabolism.getProteinHealingMultiplier(proteins)
    local deficiency = Metabolism.getMacroDeficiencyProgress("proteins", proteins)
    return lerp(1.0, 1.0 - Metabolism.MACRO_MAX_EFFECT.protein_healing, deficiency)
end
...
function Metabolism.getMacroEffectSnapshot(state, workload)
    ...
    local summary = Metabolism.normalizeWorkload(workload)
    return {
        carbDeficiency = Metabolism.getMacroDeficiencyProgress("carbs", state.carbs),
        fatDeficiency = Metabolism.getMacroDeficiencyProgress("fats", state.fats),
        proteinDeficiency = Metabolism.getMacroDeficiencyProgress("proteins", state.proteins),
        carbEnduranceMultiplier = Metabolism.getCarbEnduranceMultiplier(state.carbs, summary.effectiveEnduranceMet),
        fatWeightLossMultiplier = Metabolism.getFatWeightLossMultiplier(state.fats, state.energyBalanceKcal, state.weightController),
...
function Metabolism.getFuelPressureFactor(fuel)
    ...
    if value >= Metabolism.FUEL_PENALTY_THRESHOLD then
        ...
        return lerp(1.0, 1.35, lowProgress)
    end
    local penaltyProgress = 1.0 - (value / Metabolism.FUEL_PENALTY_THRESHOLD)
    return lerp(1.35, 1.8, penaltyProgress)
end
...
function Metabolism.getHungerGateMultiplier(fuel)
    ...
    if value >= Metabolism.FUEL_PENALTY_THRESHOLD then
        local lowProgress = (Metabolism.FUEL_LOW_THRESHOLD - value) / (Metabolism.FUEL_LOW_THRESHOLD - Metabolism.FUEL_PENALTY_THRESHOLD)
        return lerp(0.6, 1.5, lowProgress)
    end
    local penaltyProgress = 1.0 - (value / Metabolism.FUEL_PENALTY_THRESHOLD)
...
function Metabolism.getSatietyBandFactor(satietyBuffer, visibleHunger)
    local hunger = clamp(visibleHunger or 0, Metabolism.VISIBLE_HUNGER_MIN, Metabolism.VISIBLE_HUNGER_MAX)
    local satietyReturnFactor = Metabolism.getSatietyReturnFactor(satietyBuffer)
    if hunger < Metabolism.HUNGER_THRESHOLD_PECKISH then
        return satietyReturnFactor
    end
    if hunger >= Metabolism.HUNGER_THRESHOLD_HUNGRY then
        return 1.0
    end

    local fade = 1.0 - ((hunger - Metabolism.HUNGER_THRESHOLD_PECKISH) / (Metabolism.HUNGER_THRESHOLD_HUNGRY - Metabolism.HUNGER_THRESHOLD_PECKISH))
...
function Metabolism.getPassiveVisibleHungerRatePerHour(state, workload)
    ...
    local satietyBandFactor = Metabolism.getSatietyBandFactor(state.satietyBuffer, hunger)
    ...
    elseif hunger < Metabolism.HUNGER_THRESHOLD_VERY_HUNGRY then
        bandMultiplier = 1.7 * fuelPressureFactor
    else
...
function Metabolism.getWeightControllerTarget(fuel, energyBalanceKcal)
    local value = clamp(fuel, Metabolism.FUEL_MIN, Metabolism.FUEL_MAX)
    if value > Metabolism.FUEL_STORAGE_THRESHOLD then
        local storageProgress = (value - Metabolism.FUEL_STORAGE_THRESHOLD) / (Metabolism.FUEL_MAX - Metabolism.FUEL_STORAGE_THRESHOLD)
        return clamp(storageProgress, 0, 1)
    end
    if value < Metabolism.FUEL_PENALTY_THRESHOLD then
        local deficitProgress = (Metabolism.FUEL_PENALTY_THRESHOLD - value) / Metabolism.FUEL_PENALTY_THRESHOLD
        return -clamp(deficitProgress, 0, 1)
    end
    local balance = tonumber(energyBalanceKcal) or 0
    return clamp(balance / 800, -0.35, 0.35)
end

function Metabolism.getDeprivationProgress(hunger)
    local h = clamp(hunger or 0, Metabolism.VISIBLE_HUNGER_MIN, Metabolism.VISIBLE_HUNGER_CAP)
    if h <= Metabolism.DEPRIVATION_ONSET then
        return 0
    end
    return clamp((h - Metabolism.DEPRIVATION_ONSET) / (Metabolism.VISIBLE_HUNGER_CAP - Metabolism.DEPRIVATION_ONSET), 0, 1)
end

function Metabolism.getExertionPenaltyMultiplier(hunger)
    local progress = Metabolism.getDeprivationProgress(hunger)
    if progress <= 0 then
        return 1.0
    end
...
function Metabolism.getFatigueAccelFactor(hunger)
    local progress = Metabolism.getDeprivationProgress(hunger)
    if progress <= 0 then
        return 1.0
    end
...
function Metabolism.getMeleeDamageMultiplier(hunger)
    local progress = Metabolism.getDeprivationProgress(hunger)
    if progress <= 0 then
        return 1.0
    end
...
function Metabolism.getWeightTrait(weightKg)
    local weight = tonumber(weightKg) or Metabolism.DEFAULT_WEIGHT_KG
    if weight <= 50 then
        return "Emaciated"
    end
    if weight <= 65 then
...
function Metabolism.ensureState(state)
    ...
    state.carbs = clamp(state.carbs or Metabolism.DEFAULT_MACROS.carbs, 0, Metabolism.MACRO_MAX.carbs)
    state.fats = clamp(state.fats or state.lipids or Metabolism.DEFAULT_MACROS.fats, 0, Metabolism.MACRO_MAX.fats)
    state.proteins = clamp(state.proteins or Metabolism.DEFAULT_MACROS.proteins, 0, Metabolism.MACRO_MAX.proteins)
    ...
    state.energyBalanceKcal = tonumber(state.energyBalanceKcal) or 0
    ...
    state.lastWeightChangeKcal = tonumber(state.lastWeightChangeKcal) or 0
    ...
    state.lastCarbDeficiency = tonumber(state.lastCarbDeficiency) or Metabolism.getMacroDeficiencyProgress("carbs", state.carbs)
    state.lastFatDeficiency = tonumber(state.lastFatDeficiency) or Metabolism.getMacroDeficiencyProgress("fats", state.fats)
    state.lastProteinDeficiency = tonumber(state.lastProteinDeficiency) or Metabolism.getMacroDeficiencyProgress("proteins", state.proteins)
    state.lastCarbEnduranceMultiplier = tonumber(state.lastCarbEnduranceMultiplier) or Metabolism.getCarbEnduranceMultiplier(state.carbs, state.lastEffectiveEnduranceMet)
    state.lastFatWeightLossMultiplier = tonumber(state.lastFatWeightLossMultiplier) or Metabolism.getFatWeightLossMultiplier(state.fats, state.energyBalanceKcal, state.weightController)
...
function Metabolism.applyFoodValues(state, values, fraction, reason)
    ...
    state.energyBalanceKcal = state.energyBalanceKcal + applied.kcal
    state.carbs = clamp(state.carbs + applied.carbs, 0, Metabolism.MACRO_MAX.carbs)
    state.fats = clamp(state.fats + applied.fats, 0, Metabolism.MACRO_MAX.fats)
    state.proteins = clamp(state.proteins + applied.proteins, 0, Metabolism.MACRO_MAX.proteins)
    ...
    return {
        ...
        energyBalanceAfter = state.energyBalanceKcal,
...
function Metabolism.advanceState(state, elapsedHours, workload, options)
    ...
    local report = {
        ...
        startEnergyBalanceKcal = state.energyBalanceKcal,
        endFuel = state.fuel,
        endZone = Metabolism.getFuelZone(state.fuel),
        endWeightKg = state.weightKg,
        endEnergyBalanceKcal = state.energyBalanceKcal,
        burnedKcal = 0,
        baseHungerGain = 0,
        ...
        peakExertionMultiplier = Metabolism.getExertionPenaltyMultiplier(state.visibleHunger),
        ...
        peakCarbDeficiency = Metabolism.getMacroDeficiencyProgress("carbs", state.carbs),
        peakFatDeficiency = Metabolism.getMacroDeficiencyProgress("fats", state.fats),
        peakProteinDeficiency = Metabolism.getMacroDeficiencyProgress("proteins", state.proteins),
        peakCarbEnduranceMultiplier = Metabolism.getCarbEnduranceMultiplier(state.carbs, normalizedWorkload.effectiveEnduranceMet),
        peakFatWeightLossMultiplier = Metabolism.getFatWeightLossMultiplier(state.fats, state.energyBalanceKcal, state.weightController),
    ...
    local burnPerHour = Metabolism.getFuelBurnPerHourFromMet(normalizedWorkload)
    local macroBurnPerHour = Metabolism.getMacroBurnPerHourFromMet(normalizedWorkload)
    local enduranceDrainPerHour = Metabolism.getEnduranceDrainPerHourFromMet(normalizedWorkload)
    local hasPendingBurn = normalizedWorkload.pendingBurnKcal ~= nil
    local hasAppliedEnduranceDrain = normalizedWorkload.appliedEnduranceDrain ~= nil
    if hasPendingBurn and totalHours > 0 then
        burnPerHour = math.max(0, normalizedWorkload.pendingBurnKcal) / totalHours
    end
    if hasAppliedEnduranceDrain then
        report.extraEnduranceDrain = math.max(0, normalizedWorkload.appliedEnduranceDrain)
    end
    ...
    for _ = 1, slices do
        ...
        state.energyBalanceKcal = state.energyBalanceKcal - metabolicBurn
        state.carbs = clamp(state.carbs - (macroBurnPerHour.carbs * sliceHours), 0, Metabolism.MACRO_MAX.carbs)
        state.fats = clamp(state.fats - (macroBurnPerHour.fats * sliceHours), 0, Metabolism.MACRO_MAX.fats)
        state.proteins = clamp(state.proteins - (macroBurnPerHour.proteins * sliceHours), 0, Metabolism.MACRO_MAX.proteins)
        report.burnedKcal = report.burnedKcal + metabolicBurn

        local carbDeficiency = Metabolism.getMacroDeficiencyProgress("carbs", state.carbs)
        local fatDeficiency = Metabolism.getMacroDeficiencyProgress("fats", state.fats)
        local proteinDeficiency = Metabolism.getMacroDeficiencyProgress("proteins", state.proteins)
        report.peakCarbDeficiency = math.max(report.peakCarbDeficiency, carbDeficiency)
        report.peakFatDeficiency = math.max(report.peakFatDeficiency, fatDeficiency)
        ...
        local carbEnduranceMultiplier = Metabolism.getCarbEnduranceMultiplier(state.carbs, normalizedWorkload.effectiveEnduranceMet)
        local exertionMultiplier = deprivationMultiplier * carbEnduranceMultiplier
        report.peakExertionMultiplier = math.max(report.peakExertionMultiplier, exertionMultiplier)
        report.peakCarbEnduranceMultiplier = math.max(report.peakCarbEnduranceMultiplier, carbEnduranceMultiplier)
        if not hasAppliedEnduranceDrain then
            local extraEnduranceDrain = enduranceDrainPerHour * exertionMultiplier * sliceHours
            report.extraEnduranceDrain = report.extraEnduranceDrain + extraEnduranceDrain
        end

        local weightTarget = Metabolism.getWeightControllerTarget(state.fuel, state.energyBalanceKcal)
        ...
        if state.weightController ~= 0 and (state.energyBalanceKcal * state.weightController) > 0 then
            local convertedKcal = state.energyBalanceKcal * math.abs(state.weightController) * (sliceHours / Metabolism.WEIGHT_RESPONSE_HOURS)
            local fatWeightLossMultiplier = Metabolism.getFatWeightLossMultiplier(state.fats, state.energyBalanceKcal, state.weightController)
            report.peakFatWeightLossMultiplier = math.max(report.peakFatWeightLossMultiplier, fatWeightLossMultiplier)
            if convertedKcal < 0 then
                convertedKcal = convertedKcal * fatWeightLossMultiplier
            end
            state.energyBalanceKcal = state.energyBalanceKcal - convertedKcal

            local weightBefore = state.weightKg
            state.weightKg = clamp(
                state.weightKg + (convertedKcal / Metabolism.WEIGHT_KCAL_PER_KG),
                Metabolism.WEIGHT_MIN_KG,
                Metabolism.WEIGHT_MAX_KG
            )

            report.weightDeltaKg = report.weightDeltaKg + (state.weightKg - weightBefore)
            report.weightChangeKcal = report.weightChangeKcal + convertedKcal
        end
    ...
    report.endEnergyBalanceKcal = state.energyBalanceKcal
    ...
    state.lastWeightChangeKcal = report.weightChangeKcal
    state.lastWeightRateKgPerWeek = report.weightRateKgPerWeek
    state.lastExertionMultiplier = report.peakExertionMultiplier
    state.lastCarbDeficiency = report.peakCarbDeficiency
    state.lastFatDeficiency = report.peakFatDeficiency
    state.lastProteinDeficiency = report.peakProteinDeficiency
    state.lastCarbEnduranceMultiplier = report.peakCarbEnduranceMultiplier
    state.lastFatWeightLossMultiplier = report.peakFatWeightLossMultiplier
